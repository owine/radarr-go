...
show_service_urls() {
    ...
    echo -e "${BLUE}Mailpit:${NC}            http://localhost:${MAILPIT_WEB_PORT:-8025}"
    echo -e ""
...
check_system_status() {
    ...
    if check_docker_service "radarr-mailpit" "Mailpit"; then
        check_service "localhost" "${MAILPIT_WEB_PORT:-8025}" "Mailpit Email Testing"
    fi
